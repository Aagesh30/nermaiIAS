...
console.log("Applying JSON test creation patch to web_portal/App.tsx...");
...
// 1. Update genMode state type and default
...
// 2. Prepend parseAndLoadJsonQuestions to extractQuestionsFromText
...
// 3. Update createTestFromExtraction check
...
// 4. Update createTestFromExtraction payload
...
// 5. Update segmented control tabs in pdf-create subTab
...
// 6. Update conditional renders for genMode === "file" vs "text" vs "json"
...
const replacementConditionalRender = `                  {genMode === "json" ? (
                    <View style={{ marginBottom: 10 }}>
                      ...
                      {showAiPromptHelper && (() => {
                        ...
                        const topic = promptTopic || "[Enter your topic below]";

                        const enTaPrompt = ` + "`" + `You are an expert at creating MCQ question papers for Indian competitive examinations (UPSC, TNPSC, APPSC, etc.).
                          ...
                          <View style={{ backgroundColor: darkMode ?"#111a11":"#f1f8e9", borderRadius: 10, padding: 14, marginBottom: 15, borderWidth: 1, borderColor: darkMode ?"#2d4a2d":"#c8e6c9"}}>
                            ...
                            <View style={{ backgroundColor: darkMode ?"#1b2a1b":"#e8f5e9", borderRadius: 6, padding: 8, flexDirection:"row", gap: 6, alignItems:"flex-start"}}>
                              ...
                              <Text style={{ fontSize: 10, color: darkMode ?"#a5d6a7":"#388e3c", flex: 1, lineHeight: 15 }}>
                                Step 1: Enter topic & question count above → Step 2: Select paper type → Step 3: Copy prompt → Step 4: Paste in ChatGPT/Gemini/Claude/Groq/DeepSeek/Perplexity → Step 5: Copy the JSON response → Step 6: Paste below and click "Parse & Load JSON Questions"
                              </Text>
                            </View>
                          </View>
                        );
...
// 7. Update submit button text label to support genMode === "json"
