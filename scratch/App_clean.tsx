...
export default function App() {
  ...
  const [newStudent, setNewStudent] = useState({ loginUsername: "", loginPassword: "", batch: "", course: "", type: "offline", totalFees: "", feesPaid: "", joiningDate: "", firstName: "", lastName: "", email: "", phone: "", rollNumber: "", admissionNumber: "", dob: "" });
  ...
  const [newNotice, setNewNotice] = useState({ title: "", content: "", priority: "normal", publishedAt: "" });
  ...
  const [testSub, setTestSub] = useState("tests");
  ...
  const renderIDCard = (student: any, theme: string, role: string, expiry: string) => {
    ...
    const availableWidth = screenWidth - 70;
    ...
    return (
      <View style={{
        ...
        <View style={{
          ...
          <View style={[styles.idCardAvatar, { backgroundColor: cardTheme + "15" }]}>
            <Ionicons name="person" size={40} color={cardTheme} />
          </View>

          <Text style={styles.idCardName}>{student.firstName} {student.lastName}</Text>
              ...
              <View style={{ gap: 10 }}>
                ...
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 15 }}>
                  <View style={{ flex: 1, gap: 4 }}>
                    ...
                    <Text style={{ fontSize: 14, fontWeight: "bold", color: "#212121" }}>{student.firstName} {student.lastName}</Text>
                  ...
                  <View style={{ width: 65, height: 75, borderWidth: 1.5, borderColor: "#bdbdbd", borderRadius: 4, backgroundColor: "#eeeeee", alignItems: "center", justifyContent: "center" }}>
  ...
  const createNotice = async () => {
    try {
      ...
      setNewNotice({ title: "", content: "", priority: "normal", publishedAt: "" });
      loadAnnouncements();
    } catch (e: any) {
  ...
  const createStudentRecord = async () => {
    ...
    try {
      ...
      setNewStudent({ loginUsername: "", loginPassword: "", batch: "", course: "", type: "offline", totalFees: "", feesPaid: "", joiningDate: "", firstName: "", lastName: "", email: "", phone: "", rollNumber: "", admissionNumber: "", dob: "" });
      loadStudents();
    } catch (e: any) {
  ...
  const submitProfileCompletion = async () => {
    const myStudent = students.find((s: any) => s.loginUsername === user.username);
            ...
            <View style={styles.authCard}>
              ...
              {authTab === "login" && (
                <>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      setUser({ role: "guest", name: "Guest", phone: "", email: "", leadId: null, hasApplied: false, userId: "guest_anonymous" });
                      setGuestTab("home");
                      loadAnnouncements();
                      loadCampaigns();
                    }}
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
          ...
          {guestTab === "freetest" && (
            <View style={{ gap: 15 }}>
              <Text style={styles.sectionTitle}>Free Test / Quiz Portal</Text>
              <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: "#c62828", backgroundColor: "#fff8f8" }]}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <Ionicons name="lock-closed-outline" size={22} color="#c62828" />
                  <Text style={{ fontWeight: "bold", color: "#c62828", fontSize: 14 }}>Access Required</Text>
                </View>
                <Text style={{ color: "#555", fontSize: 13, lineHeight: 20, marginBottom: 12 }}>
                  Free mock tests and quiz portals are available to registered users. Sign up to unlock practice tests, track your scores, and compete on the leaderboard.
                </Text>
                <TouchableOpacity
                  onPress={() => setGuestTab("register")}
                  style={[styles.primaryBtn, { backgroundColor: "#c62828" }]}
                >
                  <Text style={styles.primaryBtnTxt}>Register Now to Access Free Tests →</Text>
                </TouchableOpacity>
              </View>

              <View style={[styles.card, { backgroundColor: "#f3f8ff" }]}>
                <Text style={{ fontWeight: "bold", color: "#1565c0", fontSize: 14, marginBottom: 10 }}>What you get after registration:</Text>
                <View style={{ gap: 8 }}>
                  {[
                    { icon: "checkmark-circle-outline", text: "Access to UPSC, TNPSC & other free mock tests" },
                    { icon: "bar-chart-outline", text: "Instant score report & detailed answer review" },
                    { icon: "trophy-outline", text: "Leaderboard ranking among all registered users" },
                    { icon: "repeat-outline", text: "Attempt tests multiple times to improve" }
                  ].map((item, i) => (
                    <View key={i} style={{ flexDirection: "row", alignItems: "flex-start", gap: 8 }}>
                      <Ionicons name={item.icon as any} size={16} color="#1565c0" style={{ marginTop: 2 }} />
                      <Text style={{ color: "#333", fontSize: 13, flex: 1 }}>{item.text}</Text>
                    </View>
                  ))}
                </View>
              </View>
      ...
      <View style={[styles.mainBody, darkMode && { backgroundColor: "#121212" }]}>
          ...
          <ScrollView style={[styles.body, darkMode && { backgroundColor: "#121212" }]} contentContainerStyle={{ paddingBottom: 80 }}>
            <View style={{ gap: 15 }}>
              ...
              {user.role === "student" && (() => {
                ...
                const hasId = myStudent.idCardGenerated === true;
                const hasTicket = myStudent.hallTicketGenerated === true;
                if (!hasId && !hasTicket) return null;

                return (
                  <View style={{ alignItems: "center", marginBottom: 20, width: "100%" }}>
                    <Text style={[styles.sectionTitle, { alignSelf: "stretch", marginBottom: 10 }]}>My Credentials & Admit Cards</Text>
                      ...
                      {hasId && (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            paddingVertical: 8,
                            borderRadius: 20,
                            backgroundColor: myCardSubTab === "idcard" ? "#c62828" : "#f5f5f5",
                            borderWidth: 1,
                            borderColor: myCardSubTab === "idcard" ? "#c62828" : "#e0e0e0"
                          }}
                        >
                          <Text style={{ color: myCardSubTab === "idcard" ? "#fff" : "#616161", fontSize: 12, fontWeight: "bold" }}>🪪 ID Card</Text>
                        </TouchableOpacity>
                      )}
                      {hasTicket && (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            paddingVertical: 8,
                            borderRadius: 20,
                            backgroundColor: myCardSubTab === "hallticket" ? "#c62828" : "#f5f5f5",
                            borderWidth: 1,
                            borderColor: myCardSubTab === "hallticket" ? "#c62828" : "#e0e0e0"
                          }}
                        >
                          <Text style={{ color: myCardSubTab === "hallticket" ? "#fff" : "#616161", fontSize: 12, fontWeight: "bold" }}>🎟️ Hall Ticket</Text>
                        </TouchableOpacity>
                      )}
                    </View>

                    {myCardSubTab === "idcard" && hasId ? (
                      <View style={{ width: "100%", justifyContent: "center", alignItems: "center", paddingVertical: 10 }}>
                        {renderIDCard(
                          myStudent,
                          myStudent.idCardTheme,
                          myStudent.idCardRole,
                          myStudent.idCardExpiry
                        )}
                      </View>
                    ) : null}

                    {myCardSubTab === "hallticket" && hasTicket ? (
                      <View style={{ width: "100%", justifyContent: "center", alignItems: "center", paddingVertical: 10 }}>
                        {renderHallTicket(
                          myStudent,
                          myStudent.hallTicketExamName,
                          myStudent.hallTicketExamDate,
                          myStudent.hallTicketVenue,
                          myStudent.hallTicketTime,
                          myStudent.hallTicketInstructions
                        )}
                      </View>
                    ) : null}
                  </View>
        ...
        {activeTab === "erp" && (
          <View style={styles.splitLayout}>
            {/* Sidebar on the left */}
            {!erpSidebarCollapsed && (
              <View style={[styles.sidebar, darkMode && styles.sidebarDark, { height: "100%", paddingHorizontal: 5 }]}>
                ...
                >
                  ...
                  {isAdmin && (
                    <>
                      ...
                      {(user.role === "super_admin" || user.role === "developer") && (
                        <>
                          <TouchableOpacity onPress={() => setErpSub("permissions")} style={[styles.sidebarTab, erpSub === "permissions" && styles.sidebarTabActive]}>
                          ...
                          <TouchableOpacity onPress={() => { setErpSub("approvals"); loadPendingApprovals(); }} style={[styles.sidebarTab, erpSub === "approvals" && styles.sidebarTabActive]}>
                      ...
                      <TouchableOpacity onPress={() => setErpSub("batch")} style={[styles.sidebarTab, erpSub === "batch" && styles.sidebarTabActive]}>
                      ...
                      <TouchableOpacity onPress={() => { setErpSub("profile-requests"); loadProfileRequests(); }} style={[styles.sidebarTab, erpSub === "profile-requests" && styles.sidebarTabActive]}>
                  ...
                  {user.role === "student" && (
                    <>
                      <TouchableOpacity onPress={() => { setErpSub("my-profile"); const myStudent = students.find((s: any) => s.loginUsername === user.username); if (myStudent) loadMyProfileRequest(myStudent.id); }} style={[styles.sidebarTab, erpSub === "my-profile" && styles.sidebarTabActive]}>
              ...
              <ScrollView style={[styles.splitContent, erpSidebarCollapsed && { paddingTop: 50 }]} contentContainerStyle={{ paddingBottom: 80 }}>
                      ...
                      <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: editingStudent ? "#0288d1" : "#c62828" }]}>
                        ...
                        {editingStudent ? (
                          <>
                            ...
                            {/* Fee Details */}
                            <Text style={{ fontWeight: "bold", color: "#0288d1", marginBottom: 6, fontSize: 13 }}>💰 Fee Details</Text>
                      ...
                      <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                        <View style={{ minWidth: 1400 }}>
                          <View style={[styles.tableRow, { backgroundColor: "#f5f5f5", paddingVertical: 8, paddingHorizontal: 12 }]}>
                            <Text style={[styles.th, { flex: 1.2, fontWeight: "bold" }]}>Name</Text>
                            <Text style={[styles.th, { flex: 1, fontWeight: "bold" }]}>Roll / Batch</Text>
                            <Text style={[styles.th, { flex: 1.2, fontWeight: "bold" }]}>Contact</Text>
                            <Text style={[styles.th, { flex: 1, fontWeight: "bold" }]}>DOB</Text>
                            <Text style={[styles.th, { flex: 1, fontWeight: "bold" }]}>Blood Group</Text>
                            <Text style={[styles.th, { flex: 1.5, fontWeight: "bold" }]}>Address</Text>
                            <Text style={[styles.th, { flex: 1.1, fontWeight: "bold" }]}>Passport Photo</Text>
                            <Text style={[styles.th, { flex: 1.1, fontWeight: "bold" }]}>Photo ID</Text>
                            <Text style={[styles.th, { flex: 1.3, fontWeight: "bold" }]}>Approved Date Time</Text>
                          ...
                          {students.length === 0 ? (
                            ...
                            students.map((s, idx) => {
                              const hasName = !!s.fullName;
                              const hasDob = !!s.dateOfBirth;
                              const hasBlood = !!s.bloodGroup;
                              ...
                              return (
                                <View key={s.id} style={[styles.tableRow, { borderBottomWidth: 1, borderColor: "#eeeeee", paddingVertical: 10, paddingHorizontal: 12, backgroundColor: idx % 2 === 0 ? "#ffffff" : "#fafafa", alignItems: "center" }]}>
                                  <Text style={{ flex: 1.2, color: hasName ? "#212121" : "#c62828", fontWeight: hasName ? "600" : "bold", fontSize: 12 }}>
                                    {s.fullName || "Pending"}
                                  </Text>
                                  <Text style={{ flex: 1, color: "#757575", fontSize: 12 }}>{s.rollNumber}{"\n"}{s.batch || "N/A"}</Text>
                                  <Text style={{ flex: 1.2, color: "#757575", fontSize: 12 }}>{s.phone || "No phone number"}</Text>
                                  <Text style={{ flex: 1, color: hasDob ? "#212121" : "#c62828", fontWeight: hasDob ? "normal" : "bold", fontSize: 12 }}>
                                    {s.dateOfBirth || "Pending"}
                                  </Text>
                                  <Text style={{ flex: 1, color: hasBlood ? "#212121" : "#c62828", fontWeight: hasBlood ? "normal" : "bold", fontSize: 12 }}>
                                    {s.bloodGroup || "Pending"}
                                  </Text>
                                  <Text style={{ flex: 1.5, color: hasAddress ? "#212121" : "#c62828", fontWeight: hasAddress ? "normal" : "bold", fontSize: 12 }}>
                                    {s.address || "Pending"}
                                  </Text>
                                  <View style={{ flex: 1.1 }}>
                                    {hasPhoto ? (
                                      <TouchableOpacity onPress={() => {
                                        if (s.photoBase64 === "test") {
                                          ...
                                          Alert.alert("Passport Photo", "File URI: [Attached image]");
                                        }
                                      }}>
                                        <Text style={{ color: "#0288d1", fontWeight: "bold", fontSize: 12, textDecorationLine: "underline" }}>View Photo</Text>
                                      </TouchableOpacity>
                                    ) : (
                                      <Text style={{ color: "#c62828", fontWeight: "bold", fontSize: 12 }}>Pending</Text>
                                    )}
                                  </View>
                                  <View style={{ flex: 1.1 }}>
                                    {hasPhotoId ? (
                                      <TouchableOpacity onPress={() => {
                                        if (s.photoIdBase64 === "test") {
                                          ...
                                          Alert.alert(s.photoIdType || "Photo ID", `File ID: [Attached ID doc]`);
                                        }
                                      }}>
                                        <Text style={{ color: "#0288d1", fontWeight: "bold", fontSize: 12, textDecorationLine: "underline" }}>View ID</Text>
                                      </TouchableOpacity>
                                    ) : (
                                      <Text style={{ color: "#c62828", fontWeight: "bold", fontSize: 12 }}>Pending</Text>
                                    )}
                                  </View>
                                  <Text style={{ flex: 1.3, color: s.approvedAt ? "#212121" : "#c62828", fontWeight: s.approvedAt ? "normal" : "bold", fontSize: 11 }}>
                                    {s.approvedAt ? new Date(s.approvedAt).toLocaleString() : "Pending"}
                                  </Text>
                ...
                {erpSub === "announcements" && (user.role === "admin" || user.role === "staff") && (() => {
                  ...
                  return (
                    <View style={{ gap: 15 }}>
                      {/* Section Tabs */}
                      <View style={{ flexDirection: "row", backgroundColor: darkMode ? "#1a1a1a" : "#f5f5f5", borderRadius: 8, padding: 4 }}>
                        <TouchableOpacity
                          onPress={() => setNoticeTab("notices")}
                          style={{
                            flex: 1,
                            paddingVertical: 10,
                            alignItems: "center",
                            backgroundColor: noticeTab === "notices" ? (darkMode ? "#333" : "#fff") : "transparent",
                            borderRadius: 6,
                            shadowColor: noticeTab === "notices" ? "#000" : "transparent",
                            shadowOffset: { width: 0, height: 1 },
                            shadowOpacity: 0.1,
                            shadowRadius: 2,
                            elevation: noticeTab === "notices" ? 2 : 0
                          }}
                        >
                          <Text style={{ fontWeight: "bold", color: noticeTab === "notices" ? "#c62828" : (darkMode ? "#aaa" : "#555"), fontSize: 13 }}>
                            📢 Notice Board
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => setNoticeTab("notifications")}
                          style={{
                            flex: 1,
                            paddingVertical: 10,
                            alignItems: "center",
                            backgroundColor: noticeTab === "notifications" ? (darkMode ? "#333" : "#fff") : "transparent",
                            borderRadius: 6,
                            shadowColor: noticeTab === "notifications" ? "#000" : "transparent",
                            shadowOffset: { width: 0, height: 1 },
                            shadowOpacity: 0.1,
                            shadowRadius: 2,
                            elevation: noticeTab === "notifications" ? 2 : 0
                          }}
                        >
                          <Text style={{ fontWeight: "bold", color: noticeTab === "notifications" ? "#c62828" : (darkMode ? "#aaa" : "#555"), fontSize: 13 }}>
                        ...
                        <>
                          ...
                          <View style={styles.card}>
                            ...
                            <TextInput
                              ...
                              onChangeText={t => setNewNotification({ ...newNotification, title: t })}
                            />
                            ...
                            <TextInput
                              ...
                              onChangeText={m => setNewNotification({ ...newNotification, message: m })}
                            />
                            ...
                            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                              {([
                                ...
                                <TouchableOpacity
                                  key={grp.key}
                                  onPress={() => setNewNotification({ ...newNotification, targetGroup: grp.key })}
                                  style={[styles.roleBtn, newNotification.targetGroup === grp.key && styles.roleBtnActive]}
                                >
                                  <Text style={[styles.roleBtnTxt, newNotification.targetGroup === grp.key && styles.roleBtnTxtActive]}>
                                    {grp.label}
                                  </Text>
                                </TouchableOpacity>
                              ))}
                            ...
                            {newNotification.targetGroup === "batch" && (
                              <View style={{ marginBottom: 15 }}>
                                <Text style={styles.label}>Select Target Batch:</Text>
                                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: "row", gap: 8, marginVertical: 6 }}>
                                  {batches.map((b: any) => {
                                    const isSelected = newNotification.targetBatch === b.batchName;
                                    return (
                                      <TouchableOpacity
                                        key={b.id}
                                        onPress={() => setNewNotification({ ...newNotification, targetBatch: b.batchName })}
                                        style={[styles.roleBtn, isSelected && styles.roleBtnActive]}
                                      >
                                        <Text style={[styles.roleBtnTxt, isSelected && styles.roleBtnTxtActive]}>
                                          {b.batchName}
                                        </Text>
                                      </TouchableOpacity>
                                    );
                            ...
                            <TouchableOpacity
                              ...
                              style={[styles.primaryBtn, isSendingNotification && { opacity: 0.7 }]}
                            >
                              <Text style={styles.primaryBtnTxt}>
                                {isSendingNotification ? "Broadcasting..." : "Broadcast Notification"}
                              </Text>
                            </TouchableOpacity>
                          </View>
                          ...
                          <View style={styles.card}>
                            <Text style={styles.sectionTitle}>Sent Broadcast Registry ({notifications.length})</Text>
                            {notifications.length === 0 ? (
                              ...
                              notifications.map((notif: any) => {
                                const formattedDate = notif.createdAt ? new Date(notif.createdAt).toLocaleDateString() : "";
                                return (
                                  <View key={notif.id} style={{ borderBottomWidth: 1, borderColor: "#eeeeee", paddingVertical: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                                    <View style={{ flex: 1, paddingRight: 10 }}>
                                      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                                        <Text style={{ fontWeight: "bold", color: "#212121", fontSize: 14 }}>{notif.title}</Text>
                                        <View style={{ backgroundColor: "#e3f2fd", paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>
                                          <Text style={{ fontSize: 9, fontWeight: "bold", color: "#1565c0" }}>
                                            {notif.targetGroup.toUpperCase()} {notif.targetBatch ? `(${notif.targetBatch})` : ""}
                                          </Text>
                                        </View>
                                      </View>
                                      <Text style={{ fontSize: 13, color: "#424242", marginTop: 4 }}>{notif.message}</Text>
                                      <Text style={{ fontSize: 11, color: "#757575", marginTop: 6 }}>
                                        Sent: {formattedDate} | By: {notif.sentBy || "Admin"}
                                      </Text>
                                    </View>
                      ...
                      return (
                        <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: "#1976d2" }]}>
                          ...
                          {Platform.OS === 'web' ? (
                            ...
                            <Text style={{ color: "#999", fontSize: 12, marginBottom: 10 }}>📱 Photo upload available on web portal.</Text>
                          )}
                          ...
                          {Platform.OS === 'web' ? (
                            ...
                            <Text style={{ color: "#999", fontSize: 12, marginBottom: 10 }}>📱 Photo ID upload available on web portal.</Text>
                          )}
                    ...
                    {user.role !== "student" ? (
                      <View style={{ gap: 15 }}>
                        ...
                        {generationMode === "batch" && (
                          <View style={styles.card}>
                            <Text style={styles.sectionTitle}>Select Target Cohort / Batch</Text>
                            <Text style={{ fontSize: 12, color: "#757575", marginBottom: 10 }}>Credentials will be generated for all active students in the selected batch.</Text>
                        ...
                        {/* Customizer Panel (Shows in Batch Mode, or in Individual Mode if student is selected) */}
                        {((generationMode === "batch") || (generationMode === "individual" && selectedIdStudent)) && (() => {
                          ...
                          const sampleStudentForPreview = generationMode === "batch"
                            ? {
                              ...
                              batch: selectedBatchForGen === "All Batches" ? "General cohort" : selectedBatchForGen
                            }
                            : selectedIdStudent;
                      ...
                      (() => {
                        const myRecord = students.find((s: any) => s.email === user.email || s.phone === user.phone || s.rollNumber === user.username || s.id === user.userId);
                        if (!myRecord) {
                          return (
                            <View style={styles.emptyContainer}>
                          ...
                          <View style={{ gap: 15, alignItems: "center", width: "100%" }}>
                            ...
                            <View style={{ flexDirection: "row", gap: 10, marginBottom: 10 }}>
                              {hasId && (
                                ...
                                >
                                  <Text style={{ color: myCardSubTab === "idcard" ? "#fff" : "#616161", fontSize: 12, fontWeight: "bold" }}>🪪 My ID Card</Text>
                                </TouchableOpacity>
                              )}
                              {hasTicket && (
                                <TouchableOpacity
                                ...
                                >
                                  <Text style={{ color: myCardSubTab === "hallticket" ? "#fff" : "#616161", fontSize: 12, fontWeight: "bold" }}>🎟️ My Hall Ticket</Text>
                                </TouchableOpacity>
                              )}
                            </View>
          ...
          <View style={styles.splitLayout}>
            ...
            {!lmsTabsCollapsed && (
              <View style={[styles.sidebar, darkMode && styles.sidebarDark, { height: "100%", paddingHorizontal: 5 }]}>
                ...
                <TouchableOpacity onPress={() => setLmsSub("quiz")} style={[styles.sidebarTab, (lmsSub === "quiz" || lmsSub === "all-quizzes" || lmsSub === "create-quiz") && styles.sidebarTabActive, (lmsSub === "quiz" || lmsSub === "all-quizzes" || lmsSub === "create-quiz") && darkMode && styles.sidebarTabActiveDark]}>
