...
class LiveSessionService {
  ...
  async fetchSession(sessionId: string): Promise<ILiveSession> {
    try {
      const response = await LiveSessionApi.getSession(sessionId);
      const session = response.data?.data || response.data;
      this.currentSession = session;
      
      // Notify subscribers with the updated session without changing state
      this.subscribers.forEach(sub => sub(this.stateMachine.getState(), this.currentSession));
  ...
  async joinSession(sessionId: string, options?: { adminOverride?: boolean }): Promise<JoinPayload> {
    try {
      ...
      const response = await LiveSessionApi.joinSession(sessionId, options?.adminOverride);
      ...
      if (detectedProvider === 'zoom' && !payload.token) {
        try {
          const tokenRes = await LiveSessionApi.generateJoinToken(sessionId, options?.adminOverride);
  ...
  async handleProviderEvent(event: ProviderIframeEvent) {
    ...
    try {
      if (event.type === 'HOST_CONNECTED') {
        console.log('[LiveSessionService] Processing HOST_CONNECTED. Current status:', this.currentSession?.status);
        ...
        if (this.currentSession?.status !== 'LIVE' && this.currentSession?.status !== 'RUNNING') {
           console.log('[LiveSessionService] Syncing HOST_CONNECTED to backend...');
           await LiveSessionApi.postEvent(event.sessionId, {
              event: 'HostConnected',
              role: event.role,
              isRemote: false
           });
           console.log('[LiveSessionService] Backend sync complete.');
