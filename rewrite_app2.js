...
if (endIdx === -1) {
  console.error("End marker not found! Trying alternate...");
  const altEndMarker = `                            );
                         })()}
                 ...
                 {/* ── LMS Recorded Classes Tab`;
  const altEndIdx = content.indexOf(altEndMarker, startIdx);
  if (altEndIdx === -1) {
      // Manual regex search
      const regexMatch = content.match(/(\s*);\s*\}\)\(\)\}\s*<\/View>\s*\}\)\s*<\/View>\s*\}\)\s*\{\/\* ── LMS Recorded Classes Tab/s);
      if(!regexMatch) {
          console.error("Regex end marker not found! Exit.");
          process.exit(1);
      }
  }
}

// Let's just use regex for end marker to be robust against spacing.
const endRegex = /;\s*\}\)\(\)\}\s*<\/View>\s*\}\)\s*<\/View>\s*\}\)\s*\{\/\* ── LMS Recorded Classes Tab.*/s;
const innerContentMatch = content.substring(startIdx).match(endRegex);
...
const endIdxRegex = startIdx + innerContentMatch.index;

const innerContent = content.substring(startIdx + startMarker.length, content.indexOf(`                           const endedCoursesMap = new Map();`, startIdx));
...
const newLogicCode = `                        {(() => {
                           ...
                           lmsLiveSessions.forEach((session: any) => {
                             ...
                             // Removed the auto-hide for isExpired && isEnded so they show up in Ended Classes
                           ...
                           const renderLiveClassCard = (session: any, topicTitle?: string) => {` + innerContent.substring(innerContent.indexOf(`{`), innerContent.lastIndexOf(`};`)) + `};
...
// Replace double tab tag 
const sanitizedNewLogicCode = newLogicCode.replace('{/* ── LMS Recorded Classes Tab                {/* ── LMS Recorded Classes Tab', '{/* ── LMS Recorded Classes Tab');

const endOfRegexIndex = endIdxRegex + innerContentMatch[0].indexOf('{/* ── LMS Recorded Classes Tab');

const newContent = content.substring(0, startIdx) + sanitizedNewLogicCode + content.substring(endOfRegexIndex + '{/* ── LMS Recorded Classes Tab (Real API Data) ───────────────────── */}'.length);

fs.writeFileSync(appPath, newContent);
console.log('App.tsx successfully updated with tabs.');
